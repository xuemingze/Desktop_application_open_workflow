...
> ✅ **状态：AI 感知功能已实现（本地模式）**
> 取消的是"外部 AI 客户端 + MCP Server"那条路（需打开 MCP、token 消耗、依赖复杂）。
> 当前采用**本地模型直连模式**：直接调用本地 Ollama API（OpenAI 兼容），
> 免费、开源、小模型（qwen2.5:7b 等）足够，无需 MCP，不消耗 token。

**两种模式对比：**

| 模式 | 依赖 | Token | 配置复杂度 |
|------|------|-------|-----------|
| ~~外部 MCP 客户端~~ | ~~MCP 服务器 + AI 客户端~~ | ~~消耗~~ | ~~高~~ |
| **本地模型（当前）** | **Ollama（本地运行）** | **免费** | **低** |

**快速启用：**
1. 安装 [Ollama](https://ollama.com)，拉取模型：`ollama pull qwen2.5:7b`
2. Ollama 默认在 `http://127.0.0.1:11434` 启动
3. 在「工具 → 系统设置 → AI 感知」填入地址，启用开关
4. 无需打开任何 MCP 服务器
