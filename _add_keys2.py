# -*- coding: utf-8 -*-
"""Safely add new keys to existing i18n.py by regenerating full dicts."""
import sys, re
from pathlib import Path
sys.stdout.reconfigure(encoding='utf-8')

# New keys to add: (key, zh, en)
NEW = [
    ("ctx_sensor_box", "\u611f\u901a\u5668\uff08\u591a\u9009\uff09", "Sensors (multi-select)"),
    ("ctx_chk_clipboard", "\ud83d\udccb \u526a\u8d34\u677f\u76d1\u542c\uff08\u63a8\u8350\u5f00\u542c\uff0c\u96f6\u5f00\u9500\uff09", "Clipboard monitor (recommended, zero overhead)"),
    ("ctx_chk_window", "\ud83d\ude3f \u524d\u53f0\u7a97\u53e3\u5207\u6362\u76d1\u542c", "Foreground window switch monitor"),
    ("ctx_chk_file", "\ud83d\udcc1 \u6587\u4ef6\u7cfb\u7edf\u76d1\u89c6\uff08\u624b\u52a8\u6dfb\u52a0\u76ee\u5f55\uff09", "File system monitor (add dirs manually)"),
    ("ctx_chk_process", "\u2699\ufe0f \u8fdb\u7a0b\u542f\u52a8/\u9000\u51fa\u76d1\u542c\uff08\u914d\u5408\u4e3b\u52a8\u5616\u89c9\u7528\u6237\u6863\u6848\u89e6\u53d1\uff09", "Process start/exit monitor (triggers proactive profile)"),
    ...
    ("ctx_proactive_hint", "\ud83d\udca1 \u5f00\u542f\u540e\uff0c\u8fdb\u7a0b\u4e8b\u4ef6\u4f1a\u6309\u300c\u4e3b\u52a8\u5616\u89c9\u6863\u6848\u300d\u89c4\u5219\u89e6\u53d1 AI \u5206\u6790\u3002\n\u9700\u8981\u540c\u65f6\u5f00\u542f\u4e0a\u65b9\u300c\u8fdb\u7a0b\u76d1\u542c\u300d\u590d\u9009\u6846\u3002", "Process events trigger AI analysis. Requires Process monitor enabled."),
...
for key, zh, en in NEW:
    ...
    # Find end of _STRINGS_ZH dict - look for the LAST '},' at start of line in the ZH section
    zh_start = txt.find('_STRINGS_ZH: dict[str, str] = {')
    en_start = txt.find('_STRINGS_EN: dict[str, str] = {')
    
    # The section between zh_start and en_start is the ZH dict
    zh_section = txt[zh_start:en_start]
    # Find the last } in zh_section (this is the dict closing brace)
    ...
    # Now find the updated EN start and its dict end
    # (text before en_start hasn't changed, so en_start is still valid)
...
print(f'Done adding {len(NEW)} keys')
