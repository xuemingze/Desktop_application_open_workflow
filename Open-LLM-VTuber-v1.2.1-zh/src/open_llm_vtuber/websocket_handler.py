...
class WebSocketHandler:
    ...
    async def _handle_bubble_event(
        self, websocket: WebSocket, client_uid: str, data: WSMessage
    ) -> None:
        """Handle bubble event from desktop-auto — broadcast to ALL connected clients
        AND inject into chat history so the VTuber knows the context when replying.
        
        Desktop-Auto sends context/notification events through this message type.
        The content is:
        1. Broadcast as full-text to all clients (frontend display + TTS)
        2. Stored as a system message in chat history (LLM context for replies)
        """
        content = data.get("content", "")
        if not content:
            logger.warning("[bubble-event] Empty content, ignoring")
            return

        logger.info(f"[bubble-event] Broadcasting: {content[:50]}...")

        # 1. Inject into chat history for ALL connected clients
        #    This ensures the VTuber's LLM knows about the pushed content when replying
        from .chat_history_manager import store_message
        system_msg = f"[系统通知] {content}"
        for uid, ctx in self.client_contexts.items():
            try:
                if ctx.history_uid:
                    store_message(
                        conf_uid=ctx.character_config.conf_uid,
                        history_uid=ctx.history_uid,
                        role="system",
                        content=system_msg,
                    )
                    logger.debug(f"[bubble-event] Injected into history for {uid}")
            except Exception as e:
                logger.warning(f"[bubble-event] Failed to inject history for {uid}: {e}")

        # 2. Broadcast full-text to ALL connected clients (including sender)
        ...
        send_tasks = []
        for uid, ws_conn in self.client_connections.items():
            try:
                send_tasks.append(ws_conn.send_text(broadcast_payload))
            except Exception as e:
                logger.warning(f"[bubble-event] Failed to send to {uid}: {e}")

        if send_tasks:
            results = await asyncio.gather(*send_tasks, return_exceptions=True)
            success = sum(1 for r in results if not isinstance(r, Exception))
            logger.info(f"[bubble-event] Sent to {success}/{len(send_tasks)} clients")
