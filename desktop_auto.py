...
class MainWindow(QMainWindow):
    ...
    # ---- 跨线程信号: VTuber 浏览器通知点击 → 主线程打开 AI 小对话 ----
    toast_click_signal = Signal(object)
    ...
    def __init__(self) -> None:
        ...
        # 桥接容器必须早于任何 _build_ui()，否则 ToolsTab 等访问 main_win._companion_bridge 会 AttributeError
        ...
        try:
            ...
            # 连接跨线程信号: VTuber 浏览器通知点击 → 主线程 context_tab 处理
            self.toast_click_signal.connect(self.context_tab._on_toast_clicked)
        ...
        # ---- AppBridges 集中容器 (Step 1B-3) ----  # 已在 __init__ 顶部预创建, 此处不重复
            ...
            def _on_browser_toast_clicked(text: str):
                ...
                try:
                    with open(r"D:\项目\控制电脑\_toast_debug.log", "a", encoding="utf-8") as _f:
                        _f.write(f"[{__import__('datetime').datetime.now():%H:%M:%S}] _on_browser_toast_clicked text={text[:60]}\n")
                except Exception:
                    pass
                intent = ToastIntent(
                    intent="VTuber 通知",
                ...
                # 改用 Signal.emit 跨线程安全地通知主线程，替代 QTimer.singleShot(0, lambda: ...)
                self.toast_click_signal.emit(intent)
    ...
    def _init_vtuber_bridge(self) -> None:
        ...
        try:
            ...
            # 主动发起 WebSocket 连接(自动重连已启用,后端随时启动均可连接)
            if config["vtuber_enabled"]:
    ...
    # ===== 调试入口:模拟 VTuber 通知被点击 + 探测状态 =====
    def _debug_log(self, msg: str) -> None:
        """调试专用:同时写日志框 + stderr + 弹窗(三路保险,防止走不进 _append_log)"""
        import sys as _sys, traceback as _tb
        _sys.stderr.write(f"[DBG] {msg}\n")
        _sys.stderr.flush()
        try:
            self._append_log(msg)
        except Exception as e:
            _sys.stderr.write(f"[DBG] _append_log 异常: {e}\n")
            _sys.stderr.flush()

    def _debug_simulate_vtuber_toast_click(self) -> None:
        """① 模拟浏览器端点击 VTuber 通知:直接走 toast_click_callback"""
        from PySide6.QtWidgets import QMessageBox
        from companion_bridge import CompanionAPIHandler
        self._debug_log("[调试①] 方法被调用 ✓")
        cb = CompanionAPIHandler.toast_click_callback
        self._debug_log(f"[调试①] callback 是否就绪: {cb is not None}")
        if not cb:
            QMessageBox.warning(self, "调试①", "callback 未设置, companion bridge 可能没起来")
            return
        sample_text = "【调试】VTuber 通知点击模拟 - 10:12"
        try:
            cb(sample_text)
            self._debug_log(f"[调试①] ✅ 回调调用完成, text={sample_text}")
            QMessageBox.information(self, "调试①", f"回调调用完成, 看到主窗口日志 [点击] 行说明 _on_browser_toast_clicked 已触发")
        except Exception as e:
            import traceback
            err = f"[调试①] ❌ 回调异常: {e}\n{traceback.format_exc()}"
            self._debug_log(err)
            QMessageBox.warning(self, "调试①", f"回调异常: {e}")

    def _debug_open_mini_chat(self) -> None:
        """② 跳过 callback 链, 直接打开 AI 小聊天窗"""
        if not hasattr(self, "context_tab"):
            from PySide6.QtWidgets import QMessageBox
            QMessageBox.warning(self, "调试②", "context_tab 不存在")
            return
        try:
            self.context_tab._open_mini_chat(None)
            self._debug_log("[调试②] ✅ _open_mini_chat 调用完成")
        except Exception as e:
            import traceback
            self._debug_log(f"[调试②] ❌ _open_mini_chat 异常: {e}\n{traceback.format_exc()}")

    def _debug_probe_toast_click_chain(self) -> None:
        """③ 探测整个链路状态 + 弹窗显示"""
        from PySide6.QtWidgets import QMessageBox
        from companion_bridge import CompanionAPIHandler
        lines = ["[调试③] ===== 链路状态探测 ====="]
        # 1) callback
        cb = CompanionAPIHandler.toast_click_callback
        lines.append(f"  callback set: {cb is not None}")
        if cb:
            lines.append(f"  callback type: {type(cb).__name__}")
        # 2) companion bridge
        bridge = getattr(self.bridges, "_companion_bridge", None) if hasattr(self, "bridges") else None
        lines.append(f"  companion bridge: {bridge is not None}")
        if bridge:
            lines.append(f"  bridge running: {getattr(bridge, '_running', '?')}")
        # 3) context_tab
        lines.append(f"  context_tab exists: {hasattr(self, 'context_tab')}")
        if hasattr(self, "context_tab"):
            ct = self.context_tab
            lines.append(f"  context_chat_tab exists: {hasattr(ct, 'context_chat_tab')}")
            dlg = getattr(ct, "_mini_chat_dialog", None)
            lines.append(f"  _mini_chat_dialog: {dlg}")
            if dlg is not None:
                lines.append(f"  _mini_chat_dialog.isVisible: {dlg.isVisible()}")
        # 4) 16260 端口检测
        try:
            import socket
            s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            s.settimeout(0.5)
            r = s.connect_ex(("127.0.0.1", 16260))
            s.close()
            lines.append(f"  16260 端口: {'LISTENING' if r == 0 else f'NOT LISTENING (r={r})'}")
        except Exception as e:
            lines.append(f"  16260 端口探测异常: {e}")
        for ln in lines:
            self._debug_log(ln)
        # 强制弹窗
        QMessageBox.information(self, "调试③ 链路状态", "\n".join(lines))
